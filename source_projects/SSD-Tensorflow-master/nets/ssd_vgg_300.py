...
SSDParams = namedtuple('SSDParameters', ['img_shape',#输入图像大小
                                         ...
                                         'anchor_sizes',#初始锚点尺寸
                                         'anchor_ratios',#锚点框长款比
                                         'anchor_steps',#特征相对于原始图的缩放
                                         'anchor_offset',
                                         'normalizations',#是否正则化
                                         'prior_scaling'#对特征图参考框向gtbox做回归时用的尺度缩放(0.1,0.1,0.2,0.2)
                                         ])
...
class SSDNet(object):
    ...
    default_params = SSDParams(
        ...
        anchor_size_bounds=[0.1, 0.9],#初始预测框大小是[45,270],即盘[0.15,0.9]
        ...
        anchor_offset=0.5,#每个锚点框中心在该特征图的cell中心,因此offset=0.5
        normalizations=[20, -1, -1, -1, -1, -1],#大于0进行归一化,仅对block4紧张归一化是因为该层比较靠前,norm较大,需做L2正则化
    ...
    def net(self, inputs,
            ...
            reuse=None,
            scope='ssd_300_vgg'):#网络名是ssd_300_vgg
        ...
        if update_feat_shapes:
            shapes = ssd_feat_shapes_from_net(r[0], self.params.feat_shapes)#根据r[0]更新shapes
    ...
    def update_feature_shapes(self, predictions):
        ...
        array).
        """
        shapes = ssd_feat_shapes_from_net(predictions, self.params.feat_shapes)#根据predictions更新shapes
        self.params = self.params._replace(feat_shapes=shapes)
    ...
    # 正负样本均衡操作,选取误差的较大的top-k作为训练的负样本，以保证正负样本比例接近1:3
...
def ssd_size_bounds_to_values(size_bounds,
    ...
    min_ratio = int(size_bounds[0] * 100)#size_bounds是默认anchor相对于原图的上下边界[10,90]
...
def ssd_anchor_one_layer(img_shape,        #检测单个特征图中所有锚点的坐标和尺寸信息（未与原图做除法）
    ...
    y, x = np.mgrid[0:feat_shape[0], 0:feat_shape[1]]
    y = (y.astype(dtype) + offset) * step / img_shape[0]#anchor中心*缩放比例/原图大小=映射到原图的中心点坐标/原图大小=cell中心相对原图的位置,阈值是[0,1]
    ...
    di = 1
    if len(sizes) > 1:   #第二个锚点框的高h[1]=sqrt（起始锚点的高*起始锚点的宽）/原图大小的高；例如：h[1]=sqrt(21*45)/300,这个框是正方形
        h[1] = math.sqrt(sizes[0] * sizes[1]) / img_shape[0]
        w[1] = math.sqrt(sizes[0] * sizes[1]) / img_shape[1]
        di += 1
    for i, r in enumerate(ratios):  #遍历长宽比例，第一个特征图，r只有两个，2和0.5；共四个锚点宽size（h[0]~h[3]）
        h[i+di] = sizes[0] / img_shape[0] / math.sqrt(r)  #高和宽是相对于原图的比例,#例如：对于第一个特征图，h[0+2]=h[2]=21/300/sqrt(2);w[0+2]=w[2]=45/300*sqrt(2)
        w[i+di] = sizes[0] / img_shape[1] * math.sqrt(r)  #例如：对于第一个特征图，h[1+2]=h[3]=21/300/sqrt(0.5);w[1+2]=w[3]=45/300*sqrt(0.5)
    return y, x, h, w
#所以,对第一个特征层来说,每个锚点四个框,一个是原始预设框的大小,一个根号(h*w)作为高和宽的正方形,一个预设尺寸/ math.sqrt(r),一个预设尺寸* math.sqrt(r)
...
def ssd_net(inputs,
    ...
    with tf.variable_scope(scope, 'ssd_300_vgg', [inputs], reuse=reuse,):
        ...
        net = slim.repeat(inputs, 2, slim.conv2d, 64, [3, 3], scope='conv1')#'block1'是重复两次卷积
        end_points['block1'] = net
        #net = slim.max_pool2d(net, [2, 2], scope='pool1')
        net = slim.conv2d(net, 64,[2, 2],stride=2, scope='conv1_g',padding='VALID')
        # Block 2.
        net = slim.repeat(net, 2, slim.conv2d, 128, [3, 3], scope='conv2')
        end_points['block2'] = net
        #net = slim.max_pool2d(net, [2, 2], scope='pool2')
        net = slim.conv2d(net, 128, [2, 2],stride=2, scope='conv2_g',padding='VALID')
        # Block 3.
        net = slim.repeat(net, 3, slim.conv2d, 256, [3, 3], scope='conv3')
        end_points['block3'] = net
        #net = slim.max_pool2d(net, [2, 2], scope='pool3')
        net = slim.conv2d(net, 256, [2, 2],stride=2, scope='conv3_g',padding='SAME')
        # Block 4.
        net = slim.repeat(net, 3, slim.conv2d, 512, [3, 3], scope='conv4')
        end_points['block4'] = net

        net = slim.conv2d(net, 512, [2, 2],stride=2, scope='conv4_g',padding='VALID')
        #net = slim.max_pool2d(net, [2, 2], scope='pool4')
        # Block 5.
        net = slim.repeat(net, 3, slim.conv2d, 512, [3, 3], scope='conv5')
        end_points['block5'] = net
        #net = slim.max_pool2d(net, [3, 3], stride=1, scope='pool5')
        net = slim.conv2d(net, 512, [3, 3], scope='conv5_g',padding='SAME')
...
def ssd_losses(logits, localisations,
    ...
    with tf.name_scope(scope, 'ssd_losses'):
        ...
        for i in range(len(logits)):
            ...
            flocalisations.append(tf.reshape(localisations[i], [-1, 4]))#预测目标边框坐标,编码形式
            fglocalisations.append(tf.reshape(glocalisations[i], [-1, 4]))#真实目标坐标,编码形式
        # And concat the crap!
        ...
        nmask = tf.logical_and(tf.logical_not(pmask),#tf逻辑与操作,非正样本中IOU<0.5的样本为先验负样本
                               gscores > -0.5)
        ...
        with tf.name_scope('cross_entropy_pos'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                  labels=gclasses)
            loss = tf.div(tf.reduce_sum(loss * fpmask), batch_size, name='value')#将置信度误差乘以正样本数求和后除以batch-size
        ...
        with tf.name_scope('localization'):
            ...
            loss = custom_layers.abs_smooth(localisations - glocalisations)#l1 smooth计算方式与正常的不一样
